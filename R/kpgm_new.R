 ...
 legacy = FALSE, W2X = TRUE) 
{
	 ...
	 if (is.null(verbose)) 
        verbose <- get("verbose", envir = .spregOptions)
    stopifnot(is.logical(verbose))
    ...
    K <- ifelse(xcolnames[1] == "(Intercept)" || all(x[, 1] == 
        ...
        instr <- cbind(WX, WWX)
        firststep <- tsls_o(y = y, yend = wy, X = x, Zinst = instr, robust = robust, legacy = legacy)
...
#        lambda.se <- sqrt(vare1*diag(res))[1]
       ...
       lambda.se <- NULL
        ...
        secstep <- tsls_o(y = yt, yend = wyt, X = xt, Zinst = instr,
