...
lag.listw <- function(x, var, zero.policy=NULL, NAOK=FALSE, ...) {
	...
	if (is.null(dim(x))) {
		...
		res <- .Call("lagw", listw$neighbours, listw$weights,
			x, as.integer(cardnb),
			as.logical(zero.policy), naok=NAOK, PACKAGE="spreg")
	} else {
...
# spreg <- function(build=FALSE) {
# #	require("utils")
	# .DESC <- packageDescription("spreg")
	# .spreg.Version <- paste(.DESC[["Package"]], ", version ", 
		# .DESC[["Version"]], ", ", .DESC[["Date"]], sep="")
	# .spreg.Build <- paste("build:", .DESC[["Built"]])
	# if (build) return(c(.spreg.Version, .spreg.Build))
	# else return(.spreg.Version)
# }
